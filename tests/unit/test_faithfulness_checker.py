...
class TestFaithfulnessChecker:
    ...
    def test_single_meaningful_token_claim_supported(self, checker):
        """Test that a claim with exactly one meaningful token is
        correctly marked as supported when that token is backed by context."""
        claim = "Python."
        context = "The candidate is a python expert with years of experience."
    ...
    def test_single_meaningful_token_claim_not_supported(self, checker):
        """Test that a claim with exactly one meaningful token is still
        marked unsupported when that token has no overlap with context."""
        claim = "Rust."
        context = "The candidate has strong Python and JavaScript skills."
